...
// ---------------------------------------------------------------------------
// sectionMesh (Task 10): cross-section outline (+ optional filled cap) of a
// mesh against a plane — off the UI thread since a quarter-million-triangle
// arch scan's per-triangle classification (../kernel/src/section/
// polyline.ts's `sectionMesh`) is real, if bounded, CPU work, and the
// optional manifold-3d cap is async WASM work already.
//
// Unlike buildBvh/measurePointToSurface/raycastMesh above, this job takes
// the mesh buffers DIRECTLY (not a cached contentHash) — a section query
// isn't a per-worker-cached "build once, query many times" workload (each
// call already needs the FULL mesh to walk every triangle once; there's no
// repeated-query structure to amortize a cache against, unlike the BVH), so
// there is no per-worker cache to keep warm and no reason to pin this to
// the size:1 measurement pool — apps/client/src/engine/section.ts runs it
// on the general (multi-worker) pool. See that module's doc for the
// engine-side call site.
//
// Cancellation/progress granularity: like buildBvh/the repair jobs above,
// this is fundamentally ONE bounded synchronous kernel call (the polyline
// extraction) plus, when requested, one bounded async manifold-3d call (the
// cap) — a single checkpoint before starting is this Phase's acceptable
// granularity (documented rationale matches those jobs' own doc comments).
// ---------------------------------------------------------------------------
...
const sectionMeshJob: JobHandler<'sectionMesh'> = async (payload, ctx) => {
