...
// ---------------------------------------------------------------------------
// 0.18.0 (Phase 5 Task 4) added the brand-new cavity/occlusalPatch.ts +
// cavity/seamDihedral.ts ops ONLY (`buildOcclusalPatch` / `measureSeamDihedral`)
// — no op in the crown chain changed. All FIVE geometry stage pins below are
// verified BYTE-IDENTICAL to 0.17.0 (this suite passes against them unmodified —
// the version guard forced exactly this documented revisit, per its own
// message). Only the crown-standin-qc pin changed, MECHANICALLY: the QcReport
// embeds `kernelVersion` (gates/report.ts), so `hashQcReport` tracks every
// version bump even when every measured value and geometry hash is unchanged —
// the unchanged geometry pins are precisely the proof this qc diff is the
// version string alone, not numerical drift. See docs/CHANGELOG-kernel.md's
// [0.18.0] entry.
const EXPECTED_KERNEL_VERSION = '0.18.0';
...
const PINNED_STAGE_HASHES: Readonly<Record<string, string>> = {
  ...
  // Changed at 0.18.0: version-string-only (QcReport embeds kernelVersion —
  // see the EXPECTED_KERNEL_VERSION comment above; geometry pins unchanged).
  'crown-standin-qc': '78bcc0f8fb3ec53e281b68c23809e0220fcaee26aa17e20f654fa24d6100ec4e',
};
